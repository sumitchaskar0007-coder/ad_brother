...
const initial = { name: '', phone: '', email: '', company: '', subject: 'Hospitality recruitment', message: '' };
...
export default function ContactPage() {
  ...
  const switchMode = (nextMode) => {
    ...
    setForm((current) => ({
      ...current,
      subject: nextMode === 'employer' ? 'Hospitality recruitment' : 'Join talent network',
    }));
  };
  const submit = (event) => {
    ...
    <>
      <section className="contact-hero">
        <div className="shell contact-hero-grid">
          <div><p className="page-number">05 / CONTACT</p><p className="kicker">Talk to AD Brothers</p><h1>Let’s find the right way forward.</h1></div>
          <div className="contact-intro"><p>Hiring, planning a new operation, or looking for your next role? Start with a conversation.</p><a href="tel:+919975978310">+91 99759 78310 <span>↗</span></a></div>
        </div>
      </section>
      ...
      <section className="contact-main">
        <div className="shell contact-main-grid">
          <div className="contact-sidebar">
            ...
            <button type="button" className={mode === 'employer' ? 'is-active' : ''} onClick={() => switchMode('employer')}><span>01</span><strong>I’m hiring</strong><b>↗</b></button>
          ...
          <form onSubmit={submit} className="editorial-form">
            <div className="form-intro"><span>{mode === 'employer' ? 'Employer enquiry' : 'Candidate introduction'}</span><h2>{mode === 'employer' ? 'Tell us about the people you need.' : 'Tell us about the work you do best.'}</h2></div>
            <div className="form-grid">
              ...
              <label className="full"><span>Subject</span><select name="subject" value={form.subject} onChange={update}>{mode === 'employer' ? <><option>Hospitality recruitment</option><option>Hotel manpower</option><option>Pre-opening team</option><option>Operations consulting</option></> : <><option>Join talent network</option><option>Management role</option><option>Culinary role</option><option>Operational role</option></>}</select></label>
              <label className="full"><span>Message *</span><textarea required rows="5" name="message" value={form.message} onChange={update} placeholder={mode === 'employer' ? 'Roles, location, timeline, and anything else we should know…' : 'Your department, experience, preferred location, and target role…'} /></label>
            </div>
